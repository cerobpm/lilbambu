...
=item $_[0] => database handle object (from DBI->connect)

=item $_[1] => HASHREF     parametros: ( "VariableCode"=> "10" , "VariableName"=> "Discharge","VariableUnitsID"=>36)

=item $_[2] => ARRAYREF  options   -U => on conflict action do update 
...
sub addVariable {
	...
	my %requiredColumns = ("VariableCode"=>"STRING","VariableName"=>"STRING");
	...
	if(!defined $_[1]->{VariableUnitsID}) {
		$_[1]->{VariableUnitsID} = 0;
	}
	...
	if(defined $res->{VariableID}) {
		...
		return "{\"status\":\"200 OK\",\"VariableID\":null}";
	}
	#~ print $stmt . "\n";
	#~ return 1;
}
...
=head2 funcion columnTypeCheck()
	...
	$_[0] => Column Types ARRAY
	$_[1] => input Columns HASH
	$_[2] => 1:allRequired, 2:checkValid, 0:none
...
sub columnTypeCheck {
	...
	foreach my $key (keys %{$_[0]})
	{
		if($_[2] == 1) {
			if(!defined $_[1]->{$key}) 
			{
				die "odm_load: Falta $key";
			} elsif ($_[1]->{$key} eq "") {
				die "odm_load: $key vacío";
			}
		}
		if($_[1]->{$key} =~ /[';]/) {
			...
			if($_[1]->{$key} eq "") {
				#~ print STDERR "par $key is null. value: ".$_[1]->{$key}.".\n";
				$types{$key}="NULL";
			} else {
				#~ print STDERR "par $key is not null. value: ".$_[1]->{$key}.".\n";
				switch ($_[0]->{$key}) {
					case "INTEGER" {
						if($_[1]->{$key} !~ /^\d+$/) {
			...
			#~ print STDERR "key $key not defined\n";
			$types{$key}="NULL";
		}
	}
	return %types;
...
sub addSource {
	my %validColumns = ("SourceCode"=>"STRING","Organization"=>"STRING","SourceDescription"=>"STRING","SourceLink"=>"STRING","ContactName"=>"STRING","Phone"=>"STRING", "Email"=>"STRING","Address"=>"STRING","City"=>"STRING","State"=>"STRING","ZipCode"=>"STRING","Citation"=>"STRING","MetadataID"=>"INTEGER");
	...
	if(defined $res->{SourceID}) {
		...
		return "{\"status\":\"200 OK\",\"SourceID\":null}";
	}
	#~ print $stmt . "\n";
	#~ return 1;
}
...
sub addMethod {
	my %validColumns = ("MethodCode"=>"STRING","MethodDescription"=>"STRING","MethodLink"=>"STRING");
	my %requiredColumns = ("MethodCode"=>"STRING","MethodDescription"=>"STRING");
	if(!defined $_[1]) {
		die "Faltan parametros";
	}
	if(ref($_[1]) ne 'HASH') {
	...
	my $stmt =qq(insert into "Methods" ($varstr) values ($valstr) on conflict (\"MethodCode\") $onConflictAction returning "SourceID");
	#~ print STDERR "$stmt\n"; 
	my $sth=$_[0]->prepare($stmt);
	my $rv=$sth->execute or die $_[0]->errstr;
	my $res = $sth->fetchrow_hashref;   #[0]->do($stmt) or die $_[0]->errstr;
	if(defined $res->{SourceID}) {
		return "{\"status\":\"200 OK\",\"MethodID\":" . $res->{SourceID} . "}";
	} else {
		return "{\"status\":\"200 OK\",\"MethodID\":null}";
	}
	#~ print $stmt . "\n";
	#~ return 1;
}
...
sub GetSources {
	my %validColumns = ("SourceID"=>"INTEGER","Organization"=>"STRING","SourceDescription"=>"STRING","SourceLink"=>"STRING","ContactName"=>"STRING","Phone"=>"STRING", "Email"=>"STRING","Address"=>"STRING","City"=>"STRING","State"=>"STRING","ZipCode"=>"STRING","Citation"=>"STRING","MetadataID"=>"INTEGER","SourceCode"=>"STRING");
	#
	# crea filtro SQL iterando parametros
	#
	my %types; 
...
=head2 funcion addSite()
	...
	$_[0] => database connection handler
	$_[1] => parameters HASH [valid params=  "SiteCode"=>"STRING"*,"SiteName"=>"STRING"*,"Latitude"=>"FLOAT"*,"Longitude"=>"FLOAT"*,"Elevation_m"=>"FLOAT","SiteType"=>"STRING","State"=>"STRING","County"=>"STRING","Comments"=>"STRING","Country"=>"STRING"      *:required
	$_[2] => options ARRAY [valid opts -U]  ]
...
sub addSite {
	...
	my $stmt =qq(
       INSERT INTO "Sites" ($varstr) 
       VALUES ($valstr) 
       ON CONFLICT (\"SiteCode\") $onConflictAction 
       RETURNING "SiteID");
	...
	if(defined $res->{SiteID}) {
		...
		return "{\"status\":\"200 OK\",\"SiteID\":null}";
	}
	#~ print $stmt . "\n";
	#~ return 1;
}
...
sub GetSites {
	...
	switch(lc($format)) {
		case "wml" {
			$stmt = "select xmlelement(name \"GetSitesResponse\",xmlattributes('http://www.cuahsi.org/waterML/1.1/' as \"xmlns\", 'http://www.w3.org/2001/XMLSchema-instance' as \"xmlns:xsi\"),xmlagg(\"SiteInfoXML\")) from \"SiteInfoXML\",\"Sites\" where \"Sites\".\"SiteID\"=\"SiteInfoXML\".\"SiteID\" $filter";
			#~ print STDERR "STATEMENT: $stmt\n";
		} else {
			die "Formato $format incorrecto. opciones: wml json fwt csv\n";
		}
...
sub GetSiteInfo {
	...
	switch(lc($format)) {
		case "wml" {
			$stmt = "SELECT xmlelement(name \"GetSiteInfoResponse\",xmlattributes('http://www.cuahsi.org/waterML/1.1/' as \"xmlns:sr\", 'http://www.w3.org/2001/XMLSchema-instance' as \"xmlns:xsi\"),\"SitesWithSeriesCatalogXML\".site)
            FROM \"SitesWithSeriesCatalogXML\",\"Sites\" 
            WHERE \"Sites\".\"SiteID\"=\"SitesWithSeriesCatalogXML\".\"SiteID\" 
	...
	if(defined $res[0]) {
		...
		return "{\"status\":\"200 OK\",\"message\":\"No se encontraron registros\"}";
	}
}
...
sub addSeries {
	...
	if(defined $_[1]->{SourceCode}) {
		...
		my $sth=$_[0]->prepare(qq(select "SourceID","Organization","SourceDescription","Citation" from "Sources" where "SourceCode"='$SourceCode'));
		my $rv=$sth->execute or die $_[0]->errstr;
		my $res = $sth->fetchrow_hashref;
		if(defined $res->{SourceID}) {
			...
			$sth=$_[0]->prepare(qq(insert into "Sources" ("SourceCode","Organization","SourceDescription","Citation") values ('$SourceCode','$Organization','$SourceDescription','$citation') returning "SourceID"));
			$rv=$sth->execute or die $_[0]->errstr;
			my $res = $sth->fetchrow_hashref;
			if(defined $res->{SourceID}) {
				$SourceID=$res->{SourceID};
	...
	if(defined $res->{SeriesID}) {
		...
		return "{\"status\":\"200 OK\",\"SeriesID\":null}";
	}
	#~ print $stmt . "\n";
	#~ return 1;
}
